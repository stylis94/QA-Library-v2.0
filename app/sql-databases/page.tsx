...
import { ArrowLeft, Database, Lightbulb, Table, Search, XCircle } from 'lucide-react'
  ...
  const executeQuery = () => {
    ...
    try {
      if (lowerCaseQuery.startsWith('select')) {
        if (lowerCaseQuery.includes('from users u join orders o on u.id = o.user_id')) {
          // Simulate a JOIN
        ...
        } else if (lowerCaseQuery.includes('from users')) {
          result = initialUsers
          ...
          result =
            'Ошибка: Неизвестная таблица или сложный запрос не поддерживается в этой песочнице.'
        }
      } else {
              ...
              <CardContent className="space-y-6">
                <Alert>
                  ...
                  <AlertDescription>
                    ...
                    {' — это хранилище, где лежат все данные системы: пользователи, заказы, продукты, транзакции. База состоит из **таблиц** (как в Excel), где есть строки и столбцы [^11].'}
                  </AlertDescription>
                </Alert>
                  ...
                  <CardContent className="space-y-2">
                    <Alert>
                      ...
                      <AlertDescription>
                        ...
                        {' — язык запросов к базе данных. QA использует SQL для проверки данных в базе [^11].'}
                      </AlertDescription>
                    </Alert>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-2">
                    ...
                    <pre className="bg-slate-100 dark:bg-slate-700 p-2 rounded text-xs overflow-auto">
                      <code>
                        {
                          "SELECT * FROM users WHERE email = 'alice@example.com'; -- Пользователь с конкретным email [^11]"
                        }
                      </code>
                    </pre>
                    <pre className="bg-slate-100 dark:bg-slate-700 p-2 rounded text-xs overflow-auto">
                      <code>
                        {
                          "SELECT * FROM users WHERE city = 'New York' AND age < 35; -- С использованием AND [^11]"
                        }
                      </code>
                    </pre>
                    <pre className="bg-slate-100 dark:bg-slate-700 p-2 rounded text-xs overflow-auto">
                      <code>
                        {
                          "SELECT * FROM users WHERE city = 'London' OR city = 'Paris'; -- С использованием OR [^11]"
                        }
                      </code>
                    </pre>
                  </CardContent>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-2">
                    ...
                    <pre className="bg-slate-100 dark:bg-slate-700 p-2 rounded text-xs overflow-auto">
                      <code>
                        {
                          "SELECT * FROM users WHERE email LIKE '%@example.com'; -- Email заканчивается на @example.com [^11]"
                        }
                      </code>
                    </pre>
                    <pre className="bg-slate-100 dark:bg-slate-700 p-2 rounded text-xs overflow-auto">
                      <code>
                        {
                          "SELECT * FROM users WHERE city IN ('New York', 'London'); -- Пользователи из Нью-Йорка или Лондона [^11]"
                        }
                      </code>
                    </pre>
                    <pre className="bg-slate-100 dark:bg-slate-700 p-2 rounded text-xs overflow-auto">
                      <code>
                        {
                          "SELECT * FROM users WHERE city NOT IN ('New York', 'London'); -- Пользователи не из Нью-Йорка и не из Лондона [^11]"
                        }
                      </code>
                    </pre>
                  </CardContent>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-2">
                    ...
                    <pre className="bg-slate-100 dark:bg-slate-700 p-2 rounded text-xs overflow-auto">
                      <code>
                        {
                          "INSERT INTO users (name, email, age, city) VALUES ('Eve', 'eve@example.com', 28, 'Berlin'); -- Добавить нового пользователя"
                        }
                      </code>
                    </pre>
                    <pre className="bg-slate-100 dark:bg-slate-700 p-2 rounded text-xs overflow-auto">
